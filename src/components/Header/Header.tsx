...
export default function Header() {
  ...
  const sections = Object.keys(t.raw("navigation"));

  const nav = {
    links: sections.map((section) => t(`navigation.${section}`)),
    ids: sections.map((section) => "#" + section)
  };

  return (
    <header
      className={`
        sticky top-0 z-10 h-24 w-full bg-rose-200
